...
event.on("dbConnected", () => {
  conDB.query(
    "CREATE DATABASE IF NOT EXISTS `winklerbingo_db` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci",
    function(err, result) {
      if (err) {
        throw err;
      }
...
event.on("dbCreated", () => {
  ...
  const stmt = // Testing if main game works stable without db
    ...
    "CREATE TABLE IF NOT EXISTS `winklerbingo_db`.`word` (`wordId` INT NOT NULL AUTO_INCREMENT, `wordText` VARCHAR(32) NOT NULL, `wordCountGuessed` INT NOT NULL DEFAULT 0, `wordCountUsed` INT NOT NULL DEFAULT 0, `wordFlagUseForAutofill` TINYINT(1) NOT NULL DEFAULT 0," +
    stamps +
...
event.on("tablesCreated", () => {
  ...
  for (let i = 0; i < startWords.length; i++) {
    stmt += `INSERT INTO winklerbingo_db.word(wordText, wordCountGuessed, wordCountUsed, wordFlagUseForAutofill) VALUES('${startWords[
      i
    ].toLowerCase()}',0,0,1);`;
  }
